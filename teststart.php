<?php
  ...
  //INSERT INTO SYS_CONTROL VALUES('TEST', '2018-09-18 13:44:00', '2018-09-19 22:44:00', 'SCHEDULED')
  $conn = new mysqli($servername, $username, $password, $dbname);
  ...
  $date->modify('-8 hours');
  $sys_start = $date->format('Y-m-d H:i:s');
  $end = str_replace("T"," ",$_POST['end']);
  $date1 = new DateTime($end);
  $date1->modify('-8 hours');
  $sys_end = $date1->format('Y-m-d H:i:s');
 
  $uni=$_SESSION['session_uni_id'];
  $sql1 = "SELECT * FROM TESTID WHERE TEST_ID= $id " ;
  $result = mysqli_query( $conn, $sql1 );
  $row = $result->fetch_assoc();
  $name=$row['NAME'];
  $insert_sql = "INSERT INTO TESTING (TEST_ID, NAME,START,SYS_START,T_END,SYS_END,UNI,CLASS) VALUES('".$id."','".$name."','".$start."','".$sys_start."','".$end."','".$sys_end."','".$uni."','".$class."')";
  $insert_success = mysqli_query($conn, $insert_sql);
  
  if($insert_success)
  {
     echo "<script>alert('SUCCESS: Test ".$id." is added into ICAS system to START at: ".$start." & END at : ".$end."')</script>";
	 //header("Location: http://icasquiz.online/page16.php");
  }
  else
  {
     echo "<script>alert('ERROR: Test ".$id." is NOT ADDED into ICAS system.')</script>";
	 //header("Location: http://icasquiz.online/page16.php");
  }
?>
    ...
    <div class="container  ">
            <div class="media-container">
                <div class="col-md-12 align-center">
                    <h1 class="mbr-section-title pb-3 mbr-white mbr-bold mbr-fonts-style display-1"><br><br>TEST HAS BEEN SET.</h1>
                    <p class="mbr-text pb-3 mbr-white mbr-fonts-style display-5">Thank you</p>
                    <div class="mbr-section-btn align-center py-2"><a class="btn btn-md btn-secondary display-4" href="page18.php">ACTIVE TEST LIST</a> <a class="btn btn-md btn-secondary display-4" href="page15.php">HOMEPAGE</a></div>
