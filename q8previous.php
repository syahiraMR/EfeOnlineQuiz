<?php
	...
	session_start();
			...
			elseif ($row['h16']=='correct' && $_POST["h16"]==0){$ans=0;}
			elseif ($row['h16']=='incorrect' && $_POST["h16"]==8){$ans=0;}
			else {$ans=1;}
			
			$array = array("$h2", "$h4", "$h6", "$h8","$h10","$h12","$h14","$h16");
			...
			else
			{
				...
				header("Location: http://icasquiz.online/first.php");
				exit;
			}
